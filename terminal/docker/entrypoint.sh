...
WS_URL="${TRADEOS_WS_URL:-ws://localhost:9001/ws}"
OUT=/usr/share/nginx/html/tradeos-config.js
...
# __TRADEOS_HOSTED__ tells the client it is NOT running behind
# app.tradeos.com, so it can stop rendering account chrome that is untrue
# here: there is no TradeOS account behind a local docker stack. The client
# defaults this to true when absent, so the hosted deploy is unaffected by its
# existence and only this file opts out.
cat > "$OUT" <<EOF
// Generated at container start from TRADEOS_WS_URL. Do not edit.
window.__TRADEOS_WS_URL__ = "${SAFE_URL}";
window.__TRADEOS_HOSTED__ = false;
EOF

echo "tradeos-terminal: feed set to ${SAFE_URL}"
exec "$@"
